...
use crate::matrix::{
    add_matrices, copy_matrix, delete_matrix, determinant, flatten_matrix, get_anti_diagonal,
    get_col_vector, get_main_diagonal, get_matrix_element, get_row_vector, identity_matrix,
    inverse_matrix, is_diagonal_matrix, is_identity_matrix, is_lo_tri_matrix, is_matrix_equal,
    is_matrix_symmetric, is_square_matrix, is_triangular_matrix, is_up_tri_matrix, is_zero_matrix,
    matrix_size, matrix_size_bytes, multiply_matrices, new_matrix, print_matrix, scale_matrix,
    set_anti_diagonal, set_col_vector, set_main_diagonal, set_matrix_element, set_row_vector,
    sub_matrix, trace_matrix, transpose_matrix, zero_matrix,
};
use crate::utils::{assert, roundn};
use crate::vector::{
    copy_vector, cross_product, delete_vector, dot_product, get_vector_element, is_unit_vector,
    is_vector_equal, is_vector_orthogonal, new_vector, set_vector_element, vector_distance,
    vector_magnitude, vector_size_bytes, zero_vector,
};
...
pub(crate) extern "C" fn test_create() -> () {
    ...
    let m: *mut Matrix = new_matrix(&raw mut data[0 as usize] as *mut f64, 2, 2);
    assert(
        (get_matrix_element(m, 0, 0) == 1 as f64
            && get_matrix_element(m, 0, 1) == 2 as f64
            && get_matrix_element(m, 1, 0) == 3 as f64
            && get_matrix_element(m, 1, 1) == 4 as f64
            && matrix_size(m) == 4
            && matrix_size_bytes(m) == 32) as i32,
    );
    delete_matrix(m);
    let mut data2: [f64; 8] = [
        1 as f64, 2 as f64, 3 as f64, 4 as f64, 5 as f64, 6 as f64, 7 as f64, 8 as f64,
    ];
    let v: *mut Vector = new_vector(&raw mut data2[0 as usize] as *mut f64, 8);
...
pub(crate) extern "C" fn test_copy() -> () {
    ...
    assert(
        (get_matrix_element(m, 0, 0) == get_matrix_element(n, 0, 0)
            ...
            && get_matrix_element(m, 1, 1) == get_matrix_element(n, 1, 1)) as i32,
    );
    delete_matrix(n);
    delete_matrix(m);
...
pub(crate) extern "C" fn test_flatten() -> () {
    unsafe { printf(c"\nTest flatten...\n".as_ptr() as *mut i8 as *const i8) };
    let mut data: [f64; 6] = [1 as f64, 2 as f64, 3 as f64, 4 as f64, 5 as f64, 6 as f64];
    let m: *mut Matrix = new_matrix(&raw mut data[0 as usize] as *mut f64, 2, 3);
    let flat: *mut Vector = flatten_matrix(m);
    assert(
        (get_vector_element(flat, 0) == 1 as f64 && get_vector_element(flat, 5) == 6 as f64) as i32,
    );
    delete_vector(flat);
    delete_matrix(m);
}
...
pub(crate) extern "C" fn test_elem() -> () {
    ...
    assert(
        (get_matrix_element(m, 0, 0) == 4 as f64
...
pub(crate) extern "C" fn test_col_vector() -> () {
    ...
    assert(
        (get_vector_element(col, 0) == 5 as f64 && get_vector_element(col, 1) == 10 as f64) as i32,
    );
...
pub(crate) extern "C" fn test_main_diagonal() -> () {
    ...
    assert((get_vector_element(v, 0) == 34 as f64 && get_vector_element(v, 1) == 56 as f64) as i32);
    delete_vector(v);
    delete_matrix(m);
    ...
    assert(
        (get_matrix_element(n, 0, 0) == 100 as f64 && get_matrix_element(n, 1, 1) == 4 as f64)
            as i32,
    );
    delete_matrix(n);
    delete_vector(w);
...
pub(crate) extern "C" fn test_anti_diagonal() -> () {
    ...
    assert(
        (get_vector_element(d, 0) == 250 as f64 && get_vector_element(d, 1) == 100 as f64) as i32,
    );
    delete_vector(d);
    delete_matrix(m);
    let n: *mut Matrix = zero_matrix(2, 2);
    ...
    assert(
        (get_matrix_element(n, 1, 0) == 9 as f64 && get_matrix_element(n, 0, 1) == 8 as f64) as i32,
    );
    delete_matrix(n);
    delete_vector(e);
}
...
pub(crate) extern "C" fn test_is_diagonal() -> () {
    unsafe { printf(c"\nTest isDiagonal...\n".as_ptr() as *mut i8 as *const i8) };
    let mut data1: [f64; 9] = [
        1 as f64, 2 as f64, 3 as f64, 0 as f64, 5 as f64, 6 as f64, 0 as f64, 0 as f64, 9 as f64,
    ];
    let m1: *mut Matrix = new_matrix(&raw mut data1[0 as usize] as *mut f64, 3, 3);
    assert(!is_diagonal_matrix(m1) as i32 as i32);
    delete_matrix(m1);
    let mut data2: [f64; 9] = [
        1 as f64, 0 as f64, 0 as f64, 0 as f64, 2 as f64, 0 as f64, 0 as f64, 0 as f64, 3 as f64,
    ];
    let m2: *mut Matrix = new_matrix(&raw mut data2[0 as usize] as *mut f64, 3, 3);
...
pub(crate) extern "C" fn test_is_triangular() -> () {
    ...
    delete_matrix(m1);
    let mut data2: [f64; 9] = [
        1 as f64, 0 as f64, 0 as f64, 4 as f64, 5 as f64, 0 as f64, 7 as f64, 8 as f64, 9 as f64,
    ];
    let m2: *mut Matrix = new_matrix(&raw mut data2[0 as usize] as *mut f64, 3, 3);
    ...
    let m3: *mut Matrix = new_matrix(&raw mut data3[0 as usize] as *mut f64, 3, 3);
...
pub(crate) extern "C" fn test_transpose() -> () {
    ...
    assert(
        (get_matrix_element(t, 0, 0) == 1 as f64
...
pub(crate) extern "C" fn test_trace() -> () {
    unsafe { printf(c"\nTest trace...\n".as_ptr() as *mut i8 as *const i8) };
    let mut data: [f64; 9] = [
        1 as f64, 2 as f64, 3 as f64, 4 as f64, 5 as f64, 6 as f64, 7 as f64, 8 as f64, 9 as f64,
    ];
    let m: *mut Matrix = new_matrix(&raw mut data[0 as usize] as *mut f64, 3, 3);
...
pub(crate) extern "C" fn test_add_matrices() -> () {
    ...
    let m1: *mut Matrix = new_matrix(&raw mut data1[0 as usize] as *mut f64, 2, 2);
    let m2: *mut Matrix = new_matrix(&raw mut data2[0 as usize] as *mut f64, 2, 2);
    let sum: *mut Matrix = add_matrices(m1, m2);
    assert(
        (get_matrix_element(sum, 0, 0) == 10 as f64
...
pub(crate) extern "C" fn test_multiply_matrices() -> () {
    ...
    assert(
        (get_matrix_element(prod, 0, 0) == 3 as f64
...
pub(crate) extern "C" fn test_scale_matrix() -> () {
    ...
    let mut data: [f64; 6] = [1 as f64, 2 as f64, 3 as f64, 4 as f64, 5 as f64, 6 as f64];
    let m: *mut Matrix = new_matrix(&raw mut data[0 as usize] as *mut f64, 3, 2);
    let scaled: *mut Matrix = scale_matrix(m, 10 as f64);
    assert(
        (get_matrix_element(scaled, 0, 0) == 10 as f64
...
pub(crate) extern "C" fn test_sub_matrix() -> () {
    ...
    assert(
        (get_matrix_element(sub, 0, 0) == 1 as f64
...
pub(crate) extern "C" fn test_determinant() -> () {
    ...
    let m1: *mut Matrix = new_matrix(&raw mut data1[0 as usize] as *mut f64, 1, 1);
    ...
    delete_matrix(m2);
    let mut data3: [f64; 9] = [
        6 as f64, 1 as f64, 1 as f64, 4 as f64, -2 as f64, 5 as f64, 2 as f64, 8 as f64, 7 as f64,
    ];
    let m3: *mut Matrix = new_matrix(&raw mut data3[0 as usize] as *mut f64, 3, 3);
    assert((determinant(m3) == -306 as f64) as i32);
    delete_matrix(m3);
    let mut data4: [f64; 16] = [
        11 as f64, 9 as f64, 24 as f64, 2 as f64, 1 as f64, 5 as f64, 2 as f64, 6 as f64, 3 as f64,
        17 as f64, 18 as f64, 1 as f64, 2 as f64, 5 as f64, 7 as f64, 1 as f64,
    ];
    let m4: *mut Matrix = new_matrix(&raw mut data4[0 as usize] as *mut f64, 4, 4);
...
pub(crate) extern "C" fn test_inverse() -> () {
    unsafe { printf(c"\nTest inverse matrix...\n".as_ptr() as *mut i8 as *const i8) };
    let mut data: [f64; 9] = [
        3 as f64, 0 as f64, 2 as f64, 2 as f64, 0 as f64, -2 as f64, 0 as f64, 1 as f64, 1 as f64,
    ];
    let m: *mut Matrix = new_matrix(&raw mut data[0 as usize] as *mut f64, 3, 3);
...
pub(crate) extern "C" fn test_vector_operations() -> () {
    ...
    let v: *mut Vector = new_vector(&raw mut data1[0 as usize] as *mut f64, 3);
    let w: *mut Vector = new_vector(&raw mut data2[0 as usize] as *mut f64, 3);
    let u1: *mut Vector = new_vector(&raw mut data3[0 as usize] as *mut f64, 3);
    let u2: *mut Vector = new_vector(&raw mut data4[0 as usize] as *mut f64, 3);
    ...
    assert(
        (get_vector_element(cross, 0) == 30 as f64
