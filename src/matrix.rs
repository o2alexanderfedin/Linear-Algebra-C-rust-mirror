...
use crate::vector::{
    assert_vector, delete_vector, fill_vector, new_vector, null_vector,
    zero_vector,
};

/// Helper function for asserting matrix and matrix data
pub(crate) extern "C" fn assert_matrix(m: *mut Matrix) -> bool {
    assert((m as *mut () != 0 as *mut () &&
                unsafe { (*m).data } as *mut () != 0 as *mut ()) as i32);
    return 1;
}
...
pub(crate) extern "C" fn null_matrix(rows: i32, cols: i32) -> *mut Matrix {
    ...
    let m: *mut Matrix =
        unsafe { malloc(core::mem::size_of::<Matrix>() as u64) } as
            *mut Matrix;
    unsafe {
        *m =
            Matrix {
...
pub(crate) extern "C" fn new_matrix(d: *mut f64, rows: i32, cols: i32)
    -> *mut Matrix {
    ...
    let mut idx: i32 = 0;
    {
        let mut i: i32 = 0;
        '__b0: loop {
            if !(i < unsafe { (*m).rows }) { break '__b0; }
            '__c0: loop {
                {
                    let mut j: i32 = 0;
                    '__b1: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b1; }
                        '__c1: loop {
                            unsafe {
                                *unsafe {
...
pub(crate) extern "C" fn fill_matrix(m: &*mut Matrix, n: f64) -> () {
    assert_matrix(*m);
    {
        let mut i: i32 = 0;
        '__b2: loop {
            if !(i < unsafe { (**m).rows }) { break '__b2; }
            '__c2: loop {
                {
                    let mut j: i32 = 0;
                    '__b3: loop {
                        if !(j < unsafe { (**m).cols }) { break '__b3; }
                        '__c3: loop {
                            unsafe {
                                *unsafe {
                                            (**m).data.offset((i * unsafe { (**m).cols } + j) as isize)
...
pub(crate) extern "C" fn identity_matrix(n: i32) -> *mut Matrix {
    let m: *mut Matrix = zero_matrix(n, n);
    {
        let mut i: i32 = 0;
        '__b4: loop {
            if !(i < unsafe { (*m).rows }) { break '__b4; }
            '__c4: loop {
                {
                    let mut j: i32 = 0;
                    '__b5: loop {
                        if !(j < unsafe { (*m).rows }) { break '__b5; }
                        '__c5: loop {
                            if i == j {
                                unsafe {
                                    *unsafe {
...
pub(crate) extern "C" fn copy_matrix(m: *mut Matrix) -> *mut Matrix {
    assert_matrix(m);
    let c: *mut Matrix =
        zero_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
    {
        let mut i: i32 = 0;
        '__b6: loop {
            if !(i < unsafe { (*m).rows }) { break '__b6; }
            '__c6: loop {
                {
                    let mut j: i32 = 0;
                    '__b7: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b7; }
                        '__c7: loop {
                            unsafe {
                                *unsafe {
...
pub(crate) extern "C" fn flatten_matrix(m: *mut Matrix) -> *mut Vector {
    assert_matrix(m);
    let flat: *mut Vector =
        null_vector(unsafe { (*m).rows } * unsafe { (*m).cols });
    let mut idx: i32 = 0;
    {
        let mut i: i32 = 0;
        '__b8: loop {
            if !(i < unsafe { (*m).rows }) { break '__b8; }
            '__c8: loop {
                {
                    let mut j: i32 = 0;
                    '__b9: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b9; }
                        '__c9: loop {
                            unsafe {
                                *unsafe {
                                            (*flat).data.offset({ let __old = idx; idx += 1; __old } as
                                                    isize)
                                        } =
                                    unsafe {
                                        *unsafe {
...
pub(crate) extern "C" fn matrix_size_bytes(m: *mut Matrix) -> i32 {
    return (core::mem::size_of::<f64>() as
                    u64).wrapping_mul(matrix_size(m) as u64) as i32;
}

/// Set element of matrix m[i,j] to scalar s
pub(crate) extern "C" fn set_matrix_element(m: *mut Matrix, i: i32, j: i32,
    s: f64) -> () {
    assert((assert_matrix(m) && i >= 0 && j >= 0 && i < unsafe { (*m).rows }
...
pub(crate) extern "C" fn get_matrix_element(m: *mut Matrix, i: i32, j: i32)
    -> f64 {
    assert((assert_matrix(m) && i >= 0 && j >= 0 && i < unsafe { (*m).rows }
                && j < unsafe { (*m).cols }) as i32);
    return unsafe {
...
pub(crate) extern "C" fn set_row_vector(m: *mut Matrix, i: i32,
    v: *mut Vector) -> () {
    assert((assert_matrix(m) && assert_vector(v) && i >= 0 &&
                i < unsafe { (*m).rows }) as i32);
    {
        let mut j: i32 = 0;
        '__b10: loop {
            if !(j < unsafe { (*v).cols }) { break '__b10; }
            '__c10: loop {
                unsafe {
                    *unsafe {
                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn get_row_vector(m: *mut Matrix, i: i32)
    -> *mut Vector {
    assert((assert_matrix(m) && i >= 0 && i < unsafe { (*m).rows }) as i32);
    let row: *mut f64 =
        unsafe {
                malloc((core::mem::size_of::<f64>() as
                            u64).wrapping_mul(unsafe { (*m).cols } as u64))
            } as *mut f64;
    {
        let mut j: i32 = 0;
        '__b11: loop {
            if !(j < unsafe { (*m).cols }) { break '__b11; }
            '__c11: loop {
                unsafe {
...
pub(crate) extern "C" fn set_col_vector(m: *mut Matrix, j: i32,
    v: *mut Vector) -> () {
    assert((assert_matrix(m) && assert_vector(v) && j >= 0 &&
                j < unsafe { (*m).cols }) as i32);
    {
        let mut i: i32 = 0;
        '__b12: loop {
            if !(i < unsafe { (*v).cols }) { break '__b12; }
            '__c12: loop {
                unsafe {
                    *unsafe {
                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn get_col_vector(m: *mut Matrix, j: i32)
    -> *mut Vector {
    assert((assert_matrix(m) && j >= 0 && j < unsafe { (*m).cols }) as i32);
    let col: *mut f64 =
        unsafe {
                malloc((core::mem::size_of::<f64>() as
                            u64).wrapping_mul(unsafe { (*m).rows } as u64))
            } as *mut f64;
    {
        let mut i: i32 = 0;
        '__b13: loop {
            if !(i < unsafe { (*m).rows }) { break '__b13; }
            '__c13: loop {
                unsafe {
...
pub(crate) extern "C" fn get_main_diagonal(m: *mut Matrix) -> *mut Vector {
            ...
            } as *mut f64;
    {
        let mut x: i32 = 0;
        '__b14: loop {
            if !(x < unsafe { (*m).rows }) { break '__b14; }
            '__c14: loop {
                unsafe {
                    *diag.offset(x as isize) =
                        unsafe {
                            *unsafe {
                                    (*m).data.offset((x * unsafe { (*m).cols } + x) as isize)
                                }
                        }
                };
                break '__c14;
...
pub(crate) extern "C" fn set_main_diagonal(m: *mut Matrix, v: *mut Vector)
    -> () {
    assert((is_square_matrix(m) && assert_vector(v) &&
                    unsafe { (*m).rows } == unsafe { (*m).cols } &&
                unsafe { (*m).cols } == unsafe { (*v).cols }) as i32);
    {
        let mut x: i32 = 0;
        '__b15: loop {
            if !(x < unsafe { (*v).cols }) { break '__b15; }
            '__c15: loop {
                unsafe {
                    *unsafe {
                                (*m).data.offset((x * unsafe { (*m).cols } + x) as isize)
                    ...
                    '__b17: loop {
                        ...
                        '__c17: loop {
                            if i + j == unsafe { (*m).rows } - 1 {
                                unsafe {
                                    *diag.offset({ let __old = x; x += 1; __old } as isize) =
                                        unsafe {
                                            *unsafe {
                                                    (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
                                                }
                                ...
                                if x == unsafe { (*m).rows } { break '__b17; }
                            }
                            break '__c17;
                        }
                        j -= 1;
                            ...
                            if i + j == unsafe { (*m).rows } - 1 {
                                unsafe {
                                    *unsafe {
                                        ...
                                        unsafe {
                                            *unsafe {
                                                    (*v).data.offset({ let __old = idx; idx += 1; __old } as
                                                            isize)
                                                }
                                        }
                                };
                                if idx == unsafe { (*m).rows } { break '__b19; }
                            }
                            break '__c19;
                        }
                        j -= 1;
...
pub(crate) extern "C" fn diagonal_product(m: *mut Matrix) -> f64 {
    ...
    let mut product: f64 = 1.0;
    {
        let mut i: i32 = 0;
        '__b20: loop {
            if !(i < unsafe { (*diagonal).cols }) { break '__b20; }
            '__c20: loop {
                product *=
                    unsafe { *unsafe { (*diagonal).data.offset(i as isize) } };
                break '__c20;
            }
            i += 1;
        }
...
pub(crate) extern "C" fn print_matrix(m: *mut Matrix, include_indices: bool)
    ...
    assert_matrix(m);
    {
        let mut i: i32 = 0;
        '__b21: loop {
            if !(i < unsafe { (*m).rows }) { break '__b21; }
            '__c21: loop {
                {
                    let mut j: i32 = 0;
                    '__b22: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b22; }
                        '__c22: loop {
                            if include_indices {
                                unsafe {
                                    printf(c"[%d,%d] -> ".as_ptr() as *mut i8 as *const i8, i,
                                        j)
                                };
                            }
                            unsafe {
                                printf(c"%8.2f ".as_ptr() as *mut i8 as *const i8,
                                    unsafe {
                                        *unsafe {
                                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
                                            }
...
pub(crate) extern "C" fn is_matrix_equal(m: *mut Matrix, n: *mut Matrix)
    -> bool {
    assert((assert_matrix(m) && assert_matrix(n)) as i32);
    if unsafe { (*m).rows } != unsafe { (*n).rows } ||
    ...
    }
    {
        let mut i: i32 = 0;
        '__b23: loop {
            if !(i < unsafe { (*m).rows }) { break '__b23; }
            '__c23: loop {
                {
                    let mut j: i32 = 0;
                    '__b24: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b24; }
                        '__c24: loop {
                            if unsafe {
                                        *unsafe {
                                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn has_same_dimensions(m: *mut Matrix, n: *mut Matrix)
    -> bool {
    assert((assert_matrix(m) && assert_matrix(n)) as i32);
...
pub(crate) extern "C" fn is_zero_matrix(m: *mut Matrix) -> bool {
    assert_matrix(m);
    {
        let mut i: i32 = 0;
        '__b25: loop {
            if !(i < unsafe { (*m).rows }) { break '__b25; }
            '__c25: loop {
                {
                    let mut j: i32 = 0;
                    '__b26: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b26; }
                        '__c26: loop {
                            if unsafe {
                                        *unsafe {
                                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn is_identity_matrix(m: *mut Matrix) -> bool {
    if !is_square_matrix(m) as i32 != 0 { return 0; }
    {
        let mut i: i32 = 0;
        '__b27: loop {
            if !(i < unsafe { (*m).rows }) { break '__b27; }
            '__c27: loop {
                {
                    let mut j: i32 = 0;
                    '__b28: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b28; }
                        '__c28: loop {
                            if i == j &&
                                    unsafe {
                                            *unsafe {
...
pub(crate) extern "C" fn is_up_tri_matrix(m: *mut Matrix) -> bool {
    assert(is_square_matrix(m) as i32);
    {
        let mut i: i32 = 0;
        '__b29: loop {
            if !(i < unsafe { (*m).rows }) { break '__b29; }
            '__c29: loop {
                {
                    let mut j: i32 = 0;
                    '__b30: loop {
...
pub(crate) extern "C" fn is_lo_tri_matrix(m: *mut Matrix) -> bool {
    assert(is_square_matrix(m) as i32);
    {
        let mut i: i32 = 0;
        '__b31: loop {
            if !(i < unsafe { (*m).rows }) { break '__b31; }
            '__c31: loop {
                {
                    let mut j: i32 = i + 1;
                    '__b32: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b32; }
                        '__c32: loop {
                            if unsafe {
                                        *unsafe {
                                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn pivot_matrix(m: *mut Matrix, mut swaps: *mut i32)
    -> *mut Matrix {
    assert(is_square_matrix(m) as i32);
    ...
    let pivot: *mut Matrix = identity_matrix(n);
    {
        ...
        '__b33: loop {
            ...
            '__c33: loop {
                let mut max: f64 =
                    unsafe {
                        *unsafe { (*m).data.offset((i * n + i) as isize) }
                    };
                let mut row: i32 = i;
                {
                    let mut j: i32 = i;
                    '__b34: loop {
                        ...
                        '__c34: loop {
                            if unsafe {
                                        *unsafe { (*m).data.offset((j * n + i) as isize) }
                                    } > max {
                                max =
                                    unsafe {
                                        *unsafe { (*m).data.offset((j * n + i) as isize) }
...
pub(crate) extern "C" fn multiply_matrices(m: *mut Matrix, n: *mut Matrix)
    -> *mut Matrix {
    assert((assert_matrix(m) && assert_matrix(n) &&
                    unsafe { (*m).cols } == unsafe { (*n).cols } &&
                unsafe { (*m).rows } == unsafe { (*n).rows }) as i32);
    let prod: *mut Matrix =
        null_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
    {
        let mut j: i32 = 0;
        '__b35: loop {
            if !(j < unsafe { (*m).rows }) { break '__b35; }
            '__c35: loop {
                {
                    let mut i: i32 = 0;
                    '__b36: loop {
                        if !(i < unsafe { (*m).cols }) { break '__b36; }
                        '__c36: loop {
                            let mut val: f64 = 0.0;
                            {
                                let mut k: i32 = 0;
                                '__b37: loop {
                                    if !(k < unsafe { (*m).cols }) { break '__b37; }
                                    '__c37: loop {
                                        val +=
                                            unsafe {
                                                    *unsafe {
                                                            (*m).data.offset((i * unsafe { (*m).cols } + k) as isize)
                                                        }
                                                } *
                                                unsafe {
                                                    *unsafe {
                                                            (*n).data.offset((k * unsafe { (*m).cols } + j) as isize)
                                                        }
                                                };
                                        break '__c37;
                                    }
...
pub(crate) extern "C" fn lu_decomposition(m: *mut Matrix, l: &mut *mut Matrix,
    u: &mut *mut Matrix, p: &mut *mut Matrix) -> i32 {
    assert((is_square_matrix(m) && *l as *mut () == 0 as *mut () &&
                    *u as *mut () == 0 as *mut () &&
    ...
    *p = pivot_matrix(m, &mut swaps);
    let m2: *mut Matrix = multiply_matrices(*p, m);
    {
        ...
        '__b38: loop {
            ...
            '__c38: loop {
                unsafe {
                    *unsafe { (**l).data.offset((j * n + j) as isize) } =
                        1 as f64
                };
                {
                    let mut i: i32 = 0;
                    '__b39: loop {
                        if !(i < j + 1) { break '__b39; }
                        '__c39: loop {
                            ...
                            unsafe {
                                *unsafe { (**u).data.offset((i * n + j) as isize) } =
                                    unsafe {
                                            *unsafe { (*m2).data.offset((i * n + j) as isize) }
                                        } - sum_u
                            };
                            break '__c39;
                        }
...
pub(crate) extern "C" fn determinant(m: *mut Matrix) -> f64 {
    ...
    if is_triangular_matrix(m) { return diagonal_product(m); }
...
pub(crate) extern "C" fn is_diagonal_matrix(m: *mut Matrix) -> bool {
    assert(is_square_matrix(m) as i32);
    {
        let mut i: i32 = 0;
        '__b44: loop {
            if !(i < unsafe { (*m).rows }) { break '__b44; }
            '__c44: loop {
                {
                    let mut j: i32 = 0;
                    '__b45: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b45; }
                        '__c45: loop {
                            if i != j &&
                                    unsafe {
                                            *unsafe {
...
pub(crate) extern "C" fn transpose_matrix(m: *mut Matrix) -> *mut Matrix {
    assert_matrix(m);
    let t: *mut Matrix =
        zero_matrix(unsafe { (*m).cols }, unsafe { (*m).rows });
    {
        let mut i: i32 = 0;
        '__b46: loop {
            if !(i < unsafe { (*m).rows }) { break '__b46; }
            '__c46: loop {
                {
                    let mut j: i32 = 0;
                    '__b47: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b47; }
                        '__c47: loop {
                            unsafe {
                                *unsafe {
...
pub(crate) extern "C" fn has_zero_row(m: *mut Matrix) -> bool {
    ...
    let mut all_zeroes: bool = 1;
    {
        let mut i: i32 = 0;
        '__b48: loop {
            if !(i < unsafe { (*m).rows }) { break '__b48; }
            '__c48: loop {
                {
                    let mut j: i32 = 0;
                    '__b49: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b49; }
                        '__c49: loop {
                            if unsafe {
                                        *unsafe {
                                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn has_zero_col(m: *mut Matrix) -> bool {
    ...
    let mut all_zeroes: bool = 1;
    {
        let mut j: i32 = 0;
        '__b50: loop {
            if !(j < unsafe { (*m).rows }) { break '__b50; }
            '__c50: loop {
                {
                    let mut i: i32 = 0;
                    '__b51: loop {
                        if !(i < unsafe { (*m).cols }) { break '__b51; }
                        '__c51: loop {
                            if unsafe {
                                        *unsafe {
                                                (*m).data.offset((i * unsafe { (*m).cols } + j) as isize)
...
pub(crate) extern "C" fn trace_matrix(m: *mut Matrix) -> f64 {
    ...
    let mut trace: f64 = 0 as f64;
    {
        let mut i: i32 = 0;
        '__b52: loop {
            if !(i < unsafe { (*m).rows }) { break '__b52; }
            '__c52: loop {
                trace +=
                    unsafe {
                        *unsafe {
                                (*m).data.offset((i * unsafe { (*m).cols } + i) as isize)
                            }
                    };
                break '__c52;
            }
...
pub(crate) extern "C" fn add_matrices(m: *mut Matrix, n: *mut Matrix)
    -> *mut Matrix {
    ...
    let mut idx: i32 = 0;
    {
        let mut i: i32 = 0;
        '__b53: loop {
            if !(i < unsafe { (*m).rows }) { break '__b53; }
            '__c53: loop {
                {
                    let mut j: i32 = 0;
                    '__b54: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b54; }
                        '__c54: loop {
                            unsafe {
...
pub(crate) extern "C" fn pow_matrix(m: *mut Matrix, k: f64) -> *mut Matrix {
        ...
        null_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
    {
        let mut i: i32 = 0;
        '__b55: loop {
            if !(i < unsafe { (*m).rows }) { break '__b55; }
            '__c55: loop {
                {
                    let mut j: i32 = 0;
                    '__b56: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b56; }
                        '__c56: loop {
                            unsafe {
                                *unsafe {
...
pub(crate) extern "C" fn scale_matrix(m: *mut Matrix, s: f64) -> *mut Matrix {
    assert_matrix(m);
    let scaled: *mut Matrix =
        null_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
    {
        let mut i: i32 = 0;
        '__b57: loop {
            if !(i < unsafe { (*m).rows }) { break '__b57; }
            '__c57: loop {
                {
                    let mut j: i32 = 0;
                    '__b58: loop {
                        if !(j < unsafe { (*m).cols }) { break '__b58; }
                        '__c58: loop {
                            unsafe {
                                *unsafe {
                                            (*scaled).data.offset((i * unsafe { (*m).cols } + j) as
                                                    isize)
                                        } =
                                    unsafe {
                                            *unsafe {
...
pub(crate) extern "C" fn reflect_axis2_d(m: *mut Matrix, axis: i32)
    -> *mut Matrix {
...
#[allow(unused_doc_comments)]
pub(crate) extern "C" fn reflect_axis3_d(m: *mut Matrix, axis: i32)
    -> *mut Matrix {
    assert((is_square_matrix(m) && unsafe { (*m).cols } == 3 && axis >= 0 &&
                axis <= 2) as i32);
    let n: *mut Matrix = null_matrix(3, 3);
    if axis == 0 {
        let mut data: [f64; 9] =
            [1 as f64, 0 as f64, 0 as f64, 0 as f64, 1 as f64, 0 as f64,
                    0 as f64, 0 as f64, -1 as f64];

        /// XY
        set_main_diagonal(m,
            new_vector(&raw mut data[0 as usize] as *mut f64, 3));
    } else if axis == 1 {
        let mut data: [f64; 9] =
            [1 as f64, 0 as f64, 0 as f64, 0 as f64, -1 as f64, 0 as f64,
                    0 as f64, 0 as f64, 1 as f64];
...
pub(crate) extern "C" fn orth_proj2_d(m: *mut Matrix, axis: i32)
    -> *mut Matrix {
    assert((is_square_matrix(m) && unsafe { (*m).cols } == 2) as i32);
    let n: *mut Matrix = zero_matrix(2, 2);
...
pub(crate) extern "C" fn orth_proj3_d(m: *mut Matrix, axis: i32)
    -> *mut Matrix {
...
pub(crate) extern "C" fn scale_n_space(m: *mut Matrix, k: f64)
    -> *mut Matrix {
    assert(is_square_matrix(m) as i32);
    let n: *mut Matrix =
        zero_matrix(unsafe { (*m).cols }, unsafe { (*m).cols });
...
pub(crate) extern "C" fn shear2_d(m: *mut Matrix, k: f64, axis: i32)
    -> *mut Matrix {
    assert((is_square_matrix(m) && unsafe { (*m).cols } == 2) as i32);
    let n: *mut Matrix = zero_matrix(2, 2);
    set_matrix_element(n, 0, 0, 1 as f64);
...
pub(crate) extern "C" fn sub_matrix(m: *mut Matrix, i: i32, j: i32)
    -> *mut Matrix {
    assert((assert_matrix(m) && i >= 0 && i < unsafe { (*m).rows } && j >= 0
                && j < unsafe { (*m).cols }) as i32);
    let sm: *mut Matrix =
        null_matrix(unsafe { (*m).rows } - 1, unsafe { (*m).cols } - 1);
    let mut idx: i32 = 0;
    {
        let mut row: i32 = 0;
        '__b60: loop {
            if !(row < unsafe { (*m).rows }) { break '__b60; }
            '__c60: loop {
                {
                    let mut col: i32 = 0;
                    '__b61: loop {
                        if !(col < unsafe { (*m).cols }) { break '__b61; }
                        '__c61: loop {
                            if row != i && col != j {
                                unsafe {
                                    *unsafe {
                                                (*sm).data.offset({ let __old = idx; idx += 1; __old } as
                                                        isize)
                                            } =
                                        unsafe {
                                            *unsafe {
                                                    (*m).data.offset((row * unsafe { (*m).cols } + col) as
                                                            isize)
                                                }
                                        }
                                };
...
pub(crate) extern "C" fn element_minor(m: *mut Matrix, i: i32, j: i32)
    -> f64 {
...
pub(crate) extern "C" fn matrix_minor(m: *mut Matrix) -> *mut Matrix {
        ...
        null_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
    {
        let mut i: i32 = 0;
        '__b62: loop {
            if !(i < unsafe { (*mm).rows }) { break '__b62; }
            '__c62: loop {
                {
                    let mut j: i32 = 0;
                    '__b63: loop {
                        if !(j < unsafe { (*mm).cols }) { break '__b63; }
                        '__c63: loop {
                            unsafe {
                                *unsafe {
...
pub(crate) extern "C" fn element_cofactor(m: *mut Matrix, i: i32, j: i32)
    -> f64 {
    return unsafe { pow(-1 as f64, (i + 1 + (j + 1)) as f64) } *
            element_minor(m, i, j);
}
...
pub(crate) extern "C" fn matrix_cofactor(m: *mut Matrix) -> *mut Matrix {
        ...
        null_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
    {
        let mut i: i32 = 0;
        '__b64: loop {
            if !(i < unsafe { (*cfm).rows }) { break '__b64; }
            '__c64: loop {
                {
                    let mut j: i32 = 0;
                    '__b65: loop {
                        if !(j < unsafe { (*cfm).cols }) { break '__b65; }
                        '__c65: loop {
                            unsafe {
                                *unsafe {
                                            (*cfm).data.offset((i * unsafe { (*cfm).cols } + j) as
                                                    isize)
...
pub(crate) extern "C" fn sign_matrix(rows: i32, cols: i32) -> *mut Matrix {
    ...
    fill_matrix(&sm, 1 as f64);
    {
        let mut i: i32 = 0;
        '__b66: loop {
            if !(i < unsafe { (*sm).rows }) { break '__b66; }
            '__c66: loop {
                {
                    let mut j: i32 = 0;
                    '__b67: loop {
                        if !(j < unsafe { (*sm).cols }) { break '__b67; }
                        '__c67: loop {
                            unsafe {
                                *unsafe {
...
pub(crate) extern "C" fn adjugate_matrix(m: *mut Matrix) -> *mut Matrix {
    ...
    let sign: *mut Matrix =
        sign_matrix(unsafe { (*m).rows }, unsafe { (*m).cols });
...
pub(crate) extern "C" fn rotate3_d(m: *mut Matrix, theta: f64, axis: i32)
    -> *mut Matrix {
    assert((is_square_matrix(m) && unsafe { (*m).cols } == 3) as i32);
    let n: *mut Matrix = zero_matrix(3, 3);
    '__s68:
